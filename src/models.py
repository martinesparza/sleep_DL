...
def get_model_cnn(channels):
    ...
    #adamw = AdamW(lr=1e-4, model=base_model)

    #model.compile(optimizer=adamw, loss=losses.sparse_categorical_crossentropy, metrics=['acc'])
    model.compile(optimizers.Adam(0.0001), losses.sparse_categorical_crossentropy, metrics=['acc'])
    model.summary()
